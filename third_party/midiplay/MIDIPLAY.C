...
*/
/* Path of the OPL instrument bank; the host program may point this elsewhere. */
const char *FMDataFile = "FM.DAT";

const char RealVoice [15] = {0, 1, 2, 3, 4, 5, 9, 10,
...
#endif
{
  ...
  if (ClockTicks > 65535) {
    ...
    #ifdef __DJGPP
    {
      /* Far-call the saved BIOS vector so the DOS clock keeps its 18.2 Hz.
         The original DJGPP port *chained* the vector instead, which ran the
         BIOS handler on every MIDI tick and made the DOS clock run 7-15x fast. */
      static struct { unsigned long off; unsigned short sel; } __attribute__((packed)) chain;
      chain.off = pm_old_handler.pm_offset;
      chain.sel = pm_old_handler.pm_selector;
      __asm__ __volatile__ ("pushfl\n\tlcall *%0" : : "m" (chain) : "memory");
    }
    #else
    asm pushf;
    BIOSTimerHandler ();
...
void InstallTimer ()
{
  ...
  //Install new timer handler (iret wrapper allocated once in InitMIDI)
  #ifdef __DJGPP
    _go32_dpmi_set_protected_mode_interrupt_vector(8, &pm_new_handler);
  #else
...
void StopMIDI ()
{
  ...
  if (Device == FM) {
    for (CurrentChannel = 0; CurrentChannel < 15; CurrentChannel++)   /* was <= 17: ran past the 15-entry InUse[] */
      if (InUse[CurrentChannel] != 0) DisableNote (RealVoice [CurrentChannel]);
...
void InitMIDI ()
{
  ...
  //Save old timer handler
    ...
    _go32_dpmi_allocate_iret_wrapper (&pm_new_handler);
  #else
